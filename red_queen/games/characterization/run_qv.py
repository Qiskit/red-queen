...
def measure_qv(qc_list, backend=None, shots=1000):
    if backend == None:
        results = [execute(qc, backend=Aer.get_backend('qasm_simulator'), shots=shots).result().get_counts() for qc in qc_list]
        return results
    else:
        results = [execute(qc, backend=backend, shots=shots).result().get_counts() for qc in qc_list]
        return results
...
def bench_qv(qubits, backend=None, trials=100, shots=1000):
    ...
    return { "quantum_volume" : 2**qubits, "heavy_percent" : (num_heavies*100)/(trials*shots), "threshold" : threshold, "passes" : passes }