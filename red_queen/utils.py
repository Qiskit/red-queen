...
class FakeFlamingo(BackendV2):
    ...
    def __init__(self, distance=3, number_of_chips=3):
        ...
        ecr_props = {}
        num_module_qubits = len(graph)
        for i in [1, 2, 3]:
            edge = (num_module_qubits * i, num_module_qubits * i - 1)
            ecr_props[edge] = InstructionProperties(
                error=rng.uniform(1e-2, 5e-1), duration=rng.uniform(1e-6, 9e-5)
            )
...
class FakeOsprey(BackendV2):
    def __init__(self):
        super().__init__()
        graph = rx.generators.directed_heavy_hex_graph(11)  # usually, 15
        ...
        measure_props = {}
        delay_props = {}
        self._target = Target("Fake Osprey like backend", num_qubits=num_qubits)
        for i in range(num_qubits):
            qarg = (i,)
            rz_props[qarg] = InstructionProperties(error=0.0, duration=0.0)
            x_props[qarg] = InstructionProperties(
